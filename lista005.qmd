---
title: "Untitlelista05d"
author: "Lucas Rocha"
format: typst
editor: visual
---

title: "lista05 atividade" output: pdf_document: default html_document: default date: "2026-08-24"
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1.20 AS NOVA_AREA
FROM PRESERVE;
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50.0) / 2 AS TAXA_AJUSTADA
FROM PRESERVE;
```
...
```{sql connection=con}
SELECT MIN(PNAME) AS PRIMEIRA_RESERVA
FROM PRESERVE;
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS SOMA_SALARIOS, 
...
```{sql connection=con}
SELECT COUNT(*) AS QTD_FUNC_DEP20
FROM EMPLOYEE
WHERE DNO = 20;
```
